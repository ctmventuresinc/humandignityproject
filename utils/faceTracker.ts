...
export class FaceTracker {
  ...
  updateFaces(detections: FaceDetection[]): TrackedFace[] {
    ...
    this.trackedFaces.forEach((trackedFace: TrackedFace) => {
      trackedFace.lastSeen++;
    });

    // Process each detection
    detections.forEach((detection: FaceDetection) => {
      ...
      if (bestMatch) {
        ...
        (bestMatch as TrackedFace).detection = detection;
        (bestMatch as TrackedFace).lastSeen = 0;
        (bestMatch as TrackedFace).confidence = Math.min((bestMatch as TrackedFace).confidence + 0.1, 1.0);
      } else {
        // Create new face
    ...
    this.trackedFaces = this.trackedFaces.filter(
      (face) => face.lastSeen < this.maxAge
    );
